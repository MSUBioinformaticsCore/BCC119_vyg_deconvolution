...
**script:** `$PROJECT/src/prefRef_scaden.R`
...
**output:** in the reqired [formats](https://scaden.readthedocs.io/en/latest/usage.html#file-formats)

* `reference_cpm.txt`   
* `reference_celltypes.txt`   

### Run scaden

**script:** `$PROJECT/src/run_scaden.R`

**arguments:**

1. Path to bulk data to be deconvoluted (.txt)  
2. Path to reference data directory   
3. reference data pattern, see the [scaden docs](https://scaden.readthedocs.io/en/latest/usage.html#scaden-simulate)     
4. Results directory    

**output:**

* `scaden_proportions.txt` containing the predicted cell type proportions.
...
# pseudobulk 
cd $PROJECT/data/Ammons_scrna/for_scaden
mkdir pseudobulk
mv $PROJECT/data/Ammons_scrna/for_scaden/reference_cpm.txt .
mv $PROJECT/data/Ammons_scrna/for_scaden/reference_celltypes.txt .

cd $PROJECT/run

sbatch $PROJECT/src/run_scaden.sh \
  $PROJECT/data/Ammons_scrna/pseudobulk/pseudobulk_filtered_genes_counts_matrix.txt \
  $PROJECT/data/Ammons_scrna/for_scaden/pseudobulk \
  "*_cpm.txt" \
  $PROJECT/results/pseudobulk
